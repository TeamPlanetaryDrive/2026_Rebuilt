...
public class AlignToAprilTag extends Command {
    ...
    public void execute() {
        ...
        // new feature -- getAllUnreadResults; getLatestResult is deprecated 
        var photonRes = photonCamera.getAllUnreadResults().get(-1);
        // theres no way this doesnt error bro
        // TODO: its using -1 wtf
        if (photonRes.hasTargets()) {
            ...
            Stream<PhotonTrackedTarget> targetOpt = photonRes.getTargets().stream().filter(t -> !t.equals(prevTarget) && t.getPoseAmbiguity() <= .2 && t.getPoseAmbiguity() != -1);

            if (targetOpt.findFirst().isPresent()) {
                PhotonTrackedTarget target = targetOpt.findFirst().get();
                ...
                Pose3d cameraPose = robotPose.transformBy(Constants.VisionConstants.cameraPositionRobotRel);
